...
bitflags! {
    ...
    pub(super) struct WindowFlags: u32 {
        const FULLSCREEN = bind::SDL_WINDOW_FULLSCREEN;
        const FULLSCREEN_DESKTOP = bind::SDL_WINDOW_FULLSCREEN_DESKTOP;
        const OPENGL = bind::SDL_WINDOW_OPENGL;
        const VULKAN = bind::SDL_WINDOW_VULKAN;
        const METAL = bind::SDL_WINDOW_METAL;
        const SHOWN = bind::SDL_WINDOW_SHOWN;
        const HIDDEN = bind::SDL_WINDOW_HIDDEN;
        const BORDERLESS = bind::SDL_WINDOW_BORDERLESS;
        const RESIZABLE = bind::SDL_WINDOW_RESIZABLE;
        const MINIMIZED = bind::SDL_WINDOW_MINIMIZED;
        const MAXIMIZED = bind::SDL_WINDOW_MAXIMIZED;
        const INPUT_GRABBED = bind::SDL_WINDOW_INPUT_GRABBED;
        const INPUT_FOCUS = bind::SDL_WINDOW_INPUT_FOCUS;
        const MOUSE_FOCUS = bind::SDL_WINDOW_MOUSE_FOCUS;
        const FOREIGN = bind::SDL_WINDOW_FOREIGN;
        const ALLOW_HIGHDPI = bind::SDL_WINDOW_ALLOW_HIGHDPI;
        const MOUSE_CAPTURE = bind::SDL_WINDOW_MOUSE_CAPTURE;
    }
}
